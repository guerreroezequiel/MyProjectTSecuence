...
static FAutoConsoleCommand GCmdGridFlowSetGoal(
	...
	FConsoleCommandWithArgsDelegate::CreateStatic([](const TArray<FString>& Args)
	{
		...
		ClearGoals(GConsoleGoals);
		AddGoalCell(GConsoleGoals, FIntPoint(X, Y));
		UE_LOG(LogTemp, Log, TEXT("Flow goal set to cell (%d,%d)"), X, Y);
	})
);

// grid.flow.mark_tile tx ty
static FAutoConsoleCommand GCmdGridFlowMarkTile(
	TEXT("grid.flow.mark_tile"),
	TEXT("Marca un tile como dirty: grid.flow.mark_tile <tileX> <tileY>"),
	FConsoleCommandWithArgsDelegate::CreateStatic([](const TArray<FString>& Args)
	{
		using namespace Grid::Flow;
		if (Args.Num() < 2) { UE_LOG(LogTemp, Warning, TEXT("Usage: grid.flow.mark_tile <tileX> <tileY>")); return; }
		int32 TX = 0, TY = 0;
		LexFromString(TX, *Args[0]);
		LexFromString(TY, *Args[1]);
		MarkTileDirty(FIntPoint(TX, TY));
		UE_LOG(LogTemp, Log, TEXT("Marked tile (%d,%d) dirty"), TX, TY);
	})
);

// grid.flow.rebuild_step [tiles] [cells]
static FAutoConsoleCommand GCmdGridFlowRebuildStep(
	TEXT("grid.flow.rebuild_step"),
	TEXT("Ejecuta un paso de rebuild: grid.flow.rebuild_step [MaxTilesPerFrame] [MaxCellsPerFrame]"),
	FConsoleCommandWithArgsDelegate::CreateStatic([](const TArray<FString>& Args)
	{
		using namespace Grid::Flow;
		EnsureDefaultGoal();
		FRebuildBudget Budget;
		if (Args.Num() >= 1) { LexFromString(Budget.MaxTilesPerFrame, *Args[0]); }
		if (Args.Num() >= 2) { LexFromString(Budget.MaxCellsPerFrame, *Args[1]); }
		RebuildStep(Budget, GConsoleGoals, GConsoleParams);
		UE_LOG(LogTemp, Log, TEXT("Rebuild step done (Tiles=%d, Cells=%d)"), Budget.MaxTilesPerFrame, Budget.MaxCellsPerFrame);
	})
);

// grid.flow.rebuild_now
static FAutoConsoleCommand GCmdGridFlowRebuildNow(
	TEXT("grid.flow.rebuild_now"),
	TEXT("Consume toda la cola de tiles dirty en varios pasos hasta vaciarla"),
	FConsoleCommandDelegate::CreateStatic([]()
	{
		using namespace Grid::Flow;
		EnsureDefaultGoal();
		FRebuildBudget Budget;
		Budget.MaxTilesPerFrame = 1024; // "infinito" práctico
		Budget.MaxCellsPerFrame = 1 << 30;
		// Consumir hasta vaciar cola
		int32 Safety = 0;
		while (!GDirtyQueue.IsEmpty() && Safety < 100000)
		{
			RebuildStep(Budget, GConsoleGoals, GConsoleParams);
			++Safety;
		}
		UE_LOG(LogTemp, Log, TEXT("Rebuild now completed"));
	})
);

// grid.flow.clear_all
static FAutoConsoleCommand GCmdGridFlowClearAll(
	TEXT("grid.flow.clear_all"),
	TEXT("Limpia almacenamiento y cola de tiles dirty"),
	FConsoleCommandDelegate::CreateStatic([]()
	{
		using namespace Grid::Flow;
		ClearAll();
		...
		UE_LOG(LogTemp, Log, TEXT("Flow storage and dirty queues cleared"));
	})
);

// grid.heat.inject x y v
static FAutoConsoleCommand GCmdGridHeatInject(
	TEXT("grid.heat.inject"),
